...
class SSD_ResNet(nn.Module):
    ...
    def __init__(self, phase, size, cfg, base, head, num_classes):
        ...
        if self.phase is 'test':
            ...
            self.detect = detect(self.num_calsses, 0, 200, 0.01, 0.25)
...
def backbone(cfg, in_planes, BatchNorm, stride=2):
    ...
    conv1 = nn.Conv1d(in_channels, 64, kernel_size=7, stride=2, padding=3,
                           bias=False)
...
def build_SSD(phase, size, num_classes, cfg):
    ...
    # mbox = [5, 5, 5, 15, 15]
    mbox = [6, 9, 12, 15, 15]
    base_, head_ = multibox(backbone(base, 1, True, 2), mbox, num_classes)
